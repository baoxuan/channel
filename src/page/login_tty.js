...
const telReg = /^1[3-9][0-9]\d{8}$/gi;
const picCodeReg = /^[A-Za-z0-9]{4}$/gi;
const seconds = 120;
...
class Login_tty extends Component {
  constructor(props) {
    super(props);
    this.state = {
      imageBase64: "",
  ...
  registerUser() {
    ...
    const passwordReg = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,20}$/gi;
    const picCodeReg = /^[A-Za-z0-9]{4}$/gi;
    const smsAuthCodeReg = /^[0-9]{4}$/gi;
...
export default withRouter(Login_tty);
